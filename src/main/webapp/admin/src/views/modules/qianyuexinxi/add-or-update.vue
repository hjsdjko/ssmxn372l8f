	...
	<div class="addEdit-block" :style='{"width":"100%","padding":"50px 0 30px 0","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.qianyuehetong" label="签约合同" prop="qianyuehetong">
					<file-upload
						tip="点击上传签约合同"
						action="file/upload"
						:limit="1"
						:type="3"
						:multiple="true"
						:fileUrls="ruleForm.qianyuehetong?ruleForm.qianyuehetong:''"
						@change="qianyuehetongUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.qianyuehetong" label="签约合同" prop="qianyuehetong">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"#2b68b5","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.qianyuehetong)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.qianyuehetong" label="签约合同" prop="qianyuehetong">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"#2b68b5","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small">无</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="签约日期" prop="qianyueriqi">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.qianyueriqi" 
						type="date"
						:readonly="ro.qianyueriqi"
						placeholder="签约日期"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.qianyueriqi" label="签约日期" prop="qianyueriqi">
					<el-input v-model="ruleForm.qianyueriqi" placeholder="签约日期" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="司机账号" prop="sijizhanghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.sijizhanghao" label="司机账号" prop="sijizhanghao">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				qianyuehetong: [
					{ required: true, message: '签约合同不能为空', trigger: 'blur' },
				],
				qianyueriqi: [
				],
				sijizhanghao: [
	...
	created() {
		this.ruleForm.qianyueriqi = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='qianyuehetong'){
							this.ruleForm.qianyuehetong = obj[o];
							this.ro.qianyuehetong = true;
							continue;
						}
						if(o=='qianyueriqi'){
							this.ruleForm.qianyueriqi = obj[o];
							this.ro.qianyueriqi = true;
							continue;
						}
						if(o=='sijizhanghao'){
	...
	if(this.ruleForm.qianyuehetong!=null) {
		this.ruleForm.qianyuehetong = this.ruleForm.qianyuehetong.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `qianyuexinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.qianyuexinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
    ...
    qianyuehetongUploadChange(fileUrls) {
	    this.ruleForm.qianyuehetong = fileUrls;
    },
  }
};
</script>
